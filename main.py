...
def make_bingo_embed(host_name: str, drawn: list[int], new: int | None) -> discord.Embed:
    ...
    for letter in "BINGO":
        ...
        embed.add_field(name=f"{letter} ({len(columns[letter])})", value=nums, inline=False)
...
class BingoView(discord.ui.View):
    ...
    @discord.ui.button(label="Claim Bingo", style=discord.ButtonStyle.success, custom_id="bingo_button", emoji="🎉")
    async def bingo_check(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if retry:
            return await interaction.response.send_message(f"Slow down! Try again in {retry:.1f}s.", ephemeral=True)

        async with game_data_lock:
            data = json_util.load_game_data()
            if str(interaction.user.id) not in data.get(self.host_id, {}).get('players', []):
        ...
        if card_numbers.issubset(drawn):
            await interaction.response.send_message("Congratulations! You got Bingo! 🎉", ephemeral=True)
            await interaction.channel.send(f":tada: {interaction.user.mention} has won the Bingo game! :tada:")
            await self.end_game()
        else:
            await interaction.response.send_message("Not quite yet. Keep trying!", ephemeral=True)

    @discord.ui.button(label="My Card", style=discord.ButtonStyle.primary, custom_id="card_button", emoji="🃏")
    async def send_card(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if os.path.exists(path):
            await interaction.response.send_message(f"Here is your Bingo card, {interaction.user.name}:", file=discord.File(path), ephemeral=True)
        else:
            await interaction.response.send_message("Sorry, I couldn't generate your card.", ephemeral=True)
...
class HostView(discord.ui.View):
    ...
    @discord.ui.button(label="Start Bingo", style=discord.ButtonStyle.success, custom_id="start_button", emoji="🚀")
    async def start_game(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with game_data_lock:
            ...
            if host_id not in data or len(data[host_id]['players']) < 2:
                return await interaction.response.send_message("Need at least two players to start.", ephemeral=True)
        ...
        await interaction.response.send_message(f"The game has begun, {interaction.user.mention}!", ephemeral=False)
        for child in self.children:
            child.disabled = True
        await interaction.message.edit(view=self)
        await self._draw_numbers(host_id, bingo_msg)

    @discord.ui.button(label="Join Game", style=discord.ButtonStyle.success, custom_id="join_button", emoji="👥")
    async def join_game(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with game_data_lock:
            ...
            if not game or game['started'] or player_id in data or len(game['players'])>= game['max_players'] or player_id in game['players']:
                return await interaction.response.send_message("Cannot join game.", ephemeral=True)
            game['players'].append(player_id)
            json_util.save_game_data(data)
            mentions = [interaction.guild.get_member(int(pid)).mention for pid in game['players']]
            embed = interaction.message.embeds[0]
            embed.set_field_at(0, name="Max Players", value=str(game['max_players']), inline=True)
    ...
    @discord.ui.button(label="Leave Game", style=discord.ButtonStyle.secondary, custom_id="leave_button", emoji="🚪")
    async def leave_game(self, interaction:discord.Interaction, button:discord.ui.Button):
        ...
        async with game_data_lock:
            ...
            if not game or player_id not in game['players'] or player_id == host_id:
                return await interaction.response.send_message("Cannot leave game.", ephemeral=True)
            game['players'].remove(player_id)
            json_util.save_game_data(data)
            mentions = [interaction.guild.get_member(int(pid)).mention for pid in game['players']]
            embed = interaction.message.embeds[0]
            embed.set_field_at(1, name="Players", value="\n".join(mentions), inline=False)
        await interaction.response.send_message(f"{interaction.user.mention} has left the game.", ephemeral=True)
        await interaction.message.edit(embed=embed, view=self)

    @discord.ui.button(label="Cancel Game", style=discord.ButtonStyle.danger, custom_id="cancel_button", emoji="❌")
    async def cancel_game(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with game_data_lock:
            ...
            json_util.save_game_data(data)
        await interaction.response.send_message("Game has been cancelled.", ephemeral=False)
...
async def bingo_host(interaction: discord.Interaction, max_players: int):
    ...
    async with game_data_lock:
        data = json_util.load_game_data()
        if user_id in data:
            return await interaction.response.send_message("You're already hosting a game.", ephemeral=True)
        data[user_id] = {"max_players": max_players, "started": False, "players": [user_id], "numbers_drawn": []}
        json_util.save_game_data(data)
